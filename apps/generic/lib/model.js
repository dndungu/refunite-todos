...
module.exports = function(){
	return {
		init: function(sandbox){
			...
			this.schema = new (sandbox.context.require('apps/generic/lib/schema.js'));
			...
			this.validator = sandbox.context.require('apps/generic/lib/validator.js');
            this.secure();
		},
		setup: function(){
...
//				that.valid = that.validate(that.data, rules, model);
		...
		insert: function(then){
			...
			self.parse(function(){
				if(!self.data[0].hasOwnProperty("handle"))
					return insert();
				var sequence = self.sandbox.context.require("utilities/sequence.js");
				var name = self.schema.getSchema().name;
				sequence.init(self.storage);
				sequence.next(name, function(error, next){
					self.data[0].handle += "-" + next.sequence;
					insert();
				});
			});
		},
